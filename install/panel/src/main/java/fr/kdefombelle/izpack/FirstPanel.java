...
public class FirstPanel extends IzPanel {
     ...
     * @param log         the log
     */
    public FirstPanel(Panel panel, InstallerFrame parent, GUIInstallData installData, Resources resources, Log log) {
     ...
     *
     * <p/>Valid layout manager are the {@link IzPanelLayout} and <tt>GridBagLayout</tt>. New panels should be use
     * IzPanelLayout. If layoutManager is null, no layout manager will be created or initialized.
     *
     * @param panel       the panel meta-data
     ...
     */
    public FirstPanel(Panel panel, InstallerFrame parent, GUIInstallData installData, LayoutManager2 layout, Resources resources) {
        ...
        String welcomeText = getString("HelloPanel.welcome1") + installData.getInfo().getAppName() + " " + installData.getInfo().getAppVersion() + getString("HelloPanel.welcome2");
        ...
        if (!authors.isEmpty()) {
            ...
            JLabel appAuthorsLabel = LabelFactory.create(authorText, parent.getIcons().get("information"), LEADING);
            ...
            for (Info.Author author : authors) {
                String email = ((author.getEmail() != null) && (author.getEmail().length() > 0)) ? (" <" +
                    author.getEmail() + ">") : "";
                label = LabelFactory.create(" - " + author.getName() + email, parent.getIcons().get("empty"), LEADING);
                add(label, NEXT_LINE);
            }
            add(IzPanelLayout.createParagraphGap());
        }
